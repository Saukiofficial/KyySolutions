...
import {
    ...
    CheckCircle, BookOpen, Youtube, Eye,
...
export default function TutorialDetail({ tutorial, relatedTutorials = [], settings }) {
    ...
    const getImageUrl = (path, fallbackText = 'Tutorial') => {
        if (!path) {
            return `https://placehold.co/1200x700/e0e7ff/4f46e5?text=${encodeURIComponent(fallbackText)}`;
        }
    ...
    const publishedRelatedTutorials = relatedTutorials.filter((item) => item?.is_published !== false);
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans text-gray-900">
            ...
            <div
                className="fixed top-0 left-0 h-1 bg-indigo-600 z-[60] transition-all duration-100"
            ...
            <div className="bg-white border-b border-gray-200 sticky top-20 z-40 shadow-sm mt-24">
                <div className="container mx-auto px-4 py-3">
                    <div className="flex items-center gap-2 text-sm text-gray-600 overflow-x-auto whitespace-nowrap">
                        ...
                        <span>›</span>
                        <Link href={route('tutorials.index')} className="hover:text-indigo-600 transition-colors">
                            Tutorial
                        </Link>
                        <span>›</span>
            ...
            <main className="container mx-auto px-4 py-8 pb-20">
                <div className="grid lg:grid-cols-12 gap-8">
                        ...
                        <article className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-gray-100">
                            ...
                            <div className="w-full bg-black aspect-video relative group rounded-xl overflow-hidden mb-8 shadow-lg border border-gray-200">
                                {videoEmbedUrl ? (
                                    ...
                                    <>
                                        <img
                                            src={getImageUrl(tutorial.thumbnail, tutorial.title || 'Tutorial')}
                                            alt={tutorial.title}
                            ...
                            <div className="mt-12 pt-8 border-t border-gray-100">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {['Learning', 'Guide', 'Step-by-step', 'Tips', tutorial.category || 'Tutorial'].map((tag, i) => (
                                        <span
                                            key={`${tag}-${i}`}
                                            className="px-4 py-1.5 bg-gray-50 text-gray-600 border border-gray-200 rounded-lg text-sm hover:border-indigo-500 hover:text-indigo-600 cursor-pointer transition-colors"
                                        >
                                            #{tag}
                                        </span>
                                    ))}
                    ...
                    <aside className="lg:col-span-4 space-y-8">
                        ...
                        {publishedRelatedTutorials.length > 0 && (
                            <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                                ...
                                <div className="space-y-5">
                                    {publishedRelatedTutorials.map((item) => (
                                        <Link
                                            key={item.id}
                                            href={route('tutorials.show', item.slug)}
                                            className="group flex gap-4 items-start"
                                        >
                                            <div className="w-24 h-16 flex-shrink-0 rounded-lg overflow-hidden relative shadow-sm border border-gray-100">
                                                <img
                                                    src={getImageUrl(item.thumbnail, item.title || 'Tutorial')}
                                                    alt={item.title}
                        ...
                        <div className="rounded-xl overflow-hidden shadow-lg relative h-72 group cursor-pointer border border-gray-100 bg-gradient-to-br from-indigo-900 to-purple-900">
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6">
                                ...
                                <p className="text-indigo-200 text-sm mb-6 max-w-xs">
                                    Konsultasi gratis dengan mentor profesional untuk memaksimalkan pembelajaran Anda
                                </p>
