...
import { Search, Calendar, ArrowRight, Clock, BookmarkPlus, PlayCircle, BookOpen } from 'lucide-react';
...
export default function TutorialIndex({ tutorials = {}, settings }) {
    ...
    const getImageUrl = (path, fallbackText = 'Tutorial') => {
        if (!path) {
            return `https://placehold.co/900x600/e0e7ff/4f46e5?text=${encodeURIComponent(fallbackText)}`;
        }
    ...
    const tutorialList = (tutorials?.data || []).filter((item) => item?.is_published !== false);
            ...
            <div className="bg-gradient-to-r from-indigo-900 via-indigo-800 to-purple-900 text-white py-3 px-4 pt-24">
                <div className="container mx-auto flex justify-between items-center text-sm">
                    <div className="flex items-center gap-4 overflow-hidden w-full md:w-auto">
                        <span className="bg-indigo-500 px-3 py-1 rounded font-bold text-xs flex-shrink-0 animate-pulse">
                            BELAJAR
                        </span>
                        <span className="truncate font-medium">
                            {latestTutorial
                                ? `Tutorial Baru: ${latestTutorial.title}`
                                : 'Tingkatkan skill digital Anda bersama kami.'}
                        </span>
                    </div>
                        ...
                        {filteredTutorials.length > 0 ? (
                            <div className="grid md:grid-cols-2 gap-6">
                                {filteredTutorials.map((tutorial) => (
                                    <Link
                                        key={tutorial.id}
                                        href={route('tutorials.show', tutorial.slug)}
                    ...
                    <aside className="lg:col-span-4 space-y-8">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {['Programming', 'Networking', 'Desain Grafis', 'Digital Marketing'].map((cat, i) => (
                                    <button
                                        ...
                                        className="px-3 py-1 bg-indigo-50 hover:bg-indigo-100 text-gray-600 hover:text-indigo-600 rounded-lg text-xs font-medium transition-colors border border-indigo-100"
                                    >
                                        {cat}
                                    </button>
                                ))}
                        ...
                        <div className="rounded-xl overflow-hidden shadow-lg relative h-64 group cursor-pointer border border-gray-100 bg-gradient-to-br from-indigo-900 to-purple-900">
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6">
                                ...
                                <p className="text-indigo-200 text-xs mb-4 max-w-[200px]">
                                    Konsultasi gratis dengan mentor profesional kami.
                                </p>

                                <Link
                                    href="/#contact"
                                    className="px-6 py-2 bg-white text-indigo-900 font-bold rounded-full text-sm hover:bg-indigo-50 transition-colors shadow-lg"
                                >
                                    Hubungi Mentor
                                </Link>
                            </div>
